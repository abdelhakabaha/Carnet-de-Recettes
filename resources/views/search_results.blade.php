    ...
    <div class="flex flex-wrap justify-around items-center mt-12">
        @foreach ($results as $result)
            <div class="rounded overflow-hidden shadow-lg text-black w-[300px]">
                <img class="w-full" src="/storage/{{ $result->image }}" alt="Image">
                <div class="px-6 py-4">
                    <div class="font-bold text-xl mb-2">{{ $result->titre }}</div>
                    <p class="text-base text-black">
                        {{ $result->ingredient }}
                    </p>
                </div>
                <div class="actions flex gap-4">
                    <form action="/delete/{{ $result->titre }}" method="post">
                        @csrf
                        @method("delete")
                    ...
                    <form action="/edit/{{ $result->titre }}" method="post">
                        @csrf
                        <input type="hidden" name="titre" value="{{ $result->titre }}">
